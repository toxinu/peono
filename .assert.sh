...
# Peono stuff
IFS=$'\n\t'
PEONO_PATH=$(cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )
PEONO="${PEONO_PATH}/peono.sh"
TESTS_CACHE="./.cache/tests"

load_tests() {
    map() { local f="$1"; shift; for i in "$@"; do "$f" "$i"; done; }
    local IFS=$'\n'
    for test_name in $(typeset -F); do
        if [[ "${test_name}" == "declare -f test_"* ]]; then
            local test_name=$(echo ${test_name} | cut -d' ' -f3)
            test(){ setup && ${test_name} && teardown; }; test
        fi
    done
}
load_tests
